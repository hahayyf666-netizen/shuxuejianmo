...
def main():
    ...
    for ver in ["aligned", "unaligned"]:
        ...
        for p in by_version[ver]:
            ...
            if not isinstance(d, dict):
                ...
                continue
            item = {"path": p.as_posix(), "keys": sorted(map(str, d.keys())), "fields": {}}
            for k, v in d.items():
                try:
        ...
        if ver == "aligned":
            ...
            for x in rows:
                fs = x.get("fields", {})
                if fs.get("text_bert",{}).get("shape") != [1,3,50] or fs.get("audio",{}).get("shape") != [1,50,74] or fs.get("vision",{}).get("shape") != [1,50,35]:
            ...
            for x in rows:
                fs=x.get("fields",{})
                if fs.get("audio",{}).get("shape") != [1,500,74] or fs.get("vision",{}).get("shape") != [1,500,35]:
                    bad_shapes.append(x["path"])
            ver_report["unexpected_key_files"] = bad_keys
            ver_report["unexpected_shape_files"] = bad_shapes
            ver_report["missing_required_for_attachment2_style_unaligned"] = ["text","text_bert","id","audio_lengths","vision_lengths"]
    ...
    (OUT/"attachment3_audit.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    lines = [
        "E题附件3数据审计","="*60,
