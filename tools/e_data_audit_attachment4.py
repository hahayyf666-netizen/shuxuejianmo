...
def version_of(path: Path) -> str:
    ...
    return "unaligned" if "unaligned" in s else "aligned" if "aligned" in s else "unknown"
...
def main():
    ...
    report={"attachment":"attachment4","root":root.as_posix(),"video_files":len(videos),"video_stems":{},"versions":{}}
    ...
    for ver in ["aligned","unaligned"]:
        ...
        for p in by_version[ver]:
            ...
            if not isinstance(d,dict):
                ...
                continue
            item={"path":p.as_posix(),"stem":p.stem,"keys":sorted(map(str,d.keys())),"fields":{}}
            for k,v in d.items():
                try:item["fields"][str(k)]=arr_meta(v)
                except Exception:item["fields"][str(k)]={"type":type(v).__name__}
            if "vision" in d:
            ...
            if "vision_lengths" in d:
                try:item["vision_length_value"]=int(np.asarray(d["vision_lengths"]).reshape(-1)[0])
                except Exception:pass
            if "audio_lengths" in d:
                try:item["audio_length_value"]=int(np.asarray(d["audio_lengths"]).reshape(-1)[0])
                except Exception:pass
            rows.append(item)
        ...
        ver_report={"count":len(rows),"keysets":{str(k):v for k,v in keysets.items()},"samples":rows}
        if ver=="aligned":
            bad_shapes=[]
            for x in rows:
                fs=x.get("fields",{})
                if fs.get("audio",{}).get("shape") not in ([50,74],[1,50,74]) or fs.get("vision",{}).get("shape") not in ([50,35],[1,50,35]):
            ...
            for x in rows:
                ...
                if "audio_length_value" in x and x["audio_length_value"]!=x.get("audio_nonzero_span"):
                    anomalies.append({
                        "version":"unaligned","path":x["path"],"type":"audio_length_vs_nonzero_span",
                        "audio_lengths":x["audio_length_value"],"audio_nonzero_span":x.get("audio_nonzero_span")
                    })
            ver_report["vision_length_vs_nonzero_span_mismatches"]=length_mismatches
            anomalies.extend({"version":"unaligned","type":"vision_length_vs_nonzero_span",**m} for m in length_mismatches)
    ...
    (OUT/"attachment4_audit.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    lines=[
        "E题附件4数据审计","="*60,
