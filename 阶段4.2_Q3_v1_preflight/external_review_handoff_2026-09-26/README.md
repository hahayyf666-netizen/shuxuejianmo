...
请以**从未参与本项目的外部审核者**身份，从原题和附件说明独立判断第三问要求，再检查本分支的实现与结果。不要把报告中的 `PASS` 当成证据。此前由同一 Codex 任务组织的子代理复核仅属内部隔离交叉检查，不替代外部审核。首轮外部4.6审核报告为 `PASS_WITH_LIMITATIONS`，要求修订交付语义后再核对；历史正式4.6 Gate 未改，4.7仍为 HOLD。

固定阅读分支：[`q3-v1-preflight`](https://github.com/hahayyf666-netizen/shuxuejianmo/tree/q3-v1-preflight)。原题、数据说明从仓库根目录和 `main` 中读取；Q3 实现以本分支的 `阶段4.2_Q3_v1_preflight/` 为准。该目录顶层 `README.md` 已区分历史与当前状态。

## 当前 finalization candidate（2026-09-26）

请先读 `q3_finalization_candidate_2026-09-26/README.md` 和 `review/Q3_4_6_R1_CANDIDATE.json`，再核查候选代码、结果和图。候选文件中的 `READY_FOR_EXTERNAL_REVIEW` 是首轮外部审核前状态；首轮审核结论为 `PASS_WITH_LIMITATIONS`，本次语义修订仍待外部核对。历史 `q3_4_6_gate.json` 保持原样；它记录的 `BLOCKED_BY_MISSING_ARTIFACT` 是更早的状态。
...
当前候选重新计算的 Attachment4 原素材证据覆盖为：分类主要参考模态 20/20、分类主导影响模态 20/20、回归主导影响模态 19/20。覆盖表示相关模态所列重要位置中**至少一处**有可回看证据，不表示全部重要位置均已映射；逐样本已验证数和已列数见 `results/final_evidence_coverage_20.csv`。test冲突统计为22/727，其中低强度21、低 margin 20；仅作描述，不调整预测。valid728核心指标在1e-6容差内复现 checkpoint 记录。T4仍为 `T4_COMPLETE_WITH_LIMITATIONS`，样本05视觉仍为 `feature_position_only`。完整原始 `aligned_50.pkl`（约1 GB）不在公开 GitHub，故无法单靠公共分支重跑完整模型推理。
...
2. **服务器和训练。** DR-X 的 CUDA preflight 通过；B0/B1 各3个种子训练；以三种子平均 valid 目标值选择架构 B0，交付种子2029由冻结协议预先固定，不从三个种子中按 valid 另选；test 未参与选择。证据见 [`服务器资格门`](../server_preflight_drx_cuda128_2026-09-26/SERVER_PREFLIGHT_CUDA128_AUDIT.md)、[`训练审计`](../formal_training_audit_drx_cuda128_2026-09-26/FORMAL_TRAINING_AUDIT_REPORT.md)、[`训练摘要`](../formal_training_audit_drx_cuda128_2026-09-26/training_summary.json)、[`模型冻结清单`](../formal_training_audit_drx_cuda128_2026-09-26/MODEL_FREEZE_MANIFEST.json)。
...
4. **原素材映射边界。** C2/T2 建立官方 aligned 行到 unaligned/来源特征行的链；T3 在部分样本上验证媒体起点；T4 仅将通过核验的具体音频/视觉重要位置局部升级为可回看的语音时段或关键帧。请查 [`C2`](../stage_c2_provenance/c2_gate.json)、[`T2`](../t2_row_boundary_and_source_inventory_2026-09-26/results/t2_aggregate_gate.json)、[`T3`](../t3_media_origin_2026-09-26/results_v3/t3_aggregate_gate.json)、[`T4`](../t4_raw_evidence_closure_2026-09-26/results/T4_GATE.json) 和 [`4.5 最终复核`](../formal_attachment4_2026-09-26/final_4_5_review/Q3_4_5_FINAL_REVIEW_REPORT.md)。文本为附件4范围内的 `verified_text`；音频和视觉历史默认 `index_only`，未通过T4的其余位置仍为 `feature_position_only` / `index_only`。T3局部成功不能自动升级正式解释。
...
2. 检查 train/valid/test 分离：scaler 是否仅从 train 拟合，架构是否按三种子平均 valid 目标值选择，交付种子2029是否预先固定，test 是否只做锁定后的评价。核对 B0 seed2029 checkpoint 与两次正式输出的 SHA。
...
- 历史预 finalization 审计曾按旧字段口径报告分类/回归主要模态各18/20条；该计数已由当前 `q3_finalization_candidate_2026-09-26/results/final_evidence_coverage_20.csv` 按冻结最终语义与T4证据重新计算并取代。当前“至少一处经验证原素材证据”的口径为 primary-reference 20/20、dominant-classification 20/20、dominant-regression 19/20；不定义 `primary_supporting_modality_reg`。
